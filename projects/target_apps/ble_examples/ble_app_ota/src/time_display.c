...
void draw_time_page(uint32_t unix_time, bool force_redraw)
{
    ...
    else
    {
        Paint_SelectImage(epd_buffer);
        Paint_SetMirroring(MIRROR_VERTICAL);
        // If not force_redraw, we might still want to clear specific areas or the whole screen
        // In the original code, digital time mode always did a full refresh or part refresh
        // For simplicity in this refactor, we usually do a full buffer rebuild
        Paint_Clear(WHITE);
    }
    ...
    if (lunar.IsLeap) {
        sprintf(buf2, "Am lich: %02d-%02d-%04d (N)", lunar.Date, lunar.Month, lunar.Year);
    } else {
        sprintf(buf2, "Am lich: %02d-%02d-%04d", lunar.Date, lunar.Month, lunar.Year);
    }
    EPD_DrawUTF8(0, 90, 0, buf2, EPD_ASCII_11X16, EPD_FontUTF8_16x16, BLACK, WHITE);
}
