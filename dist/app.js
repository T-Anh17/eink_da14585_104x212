...
function getBluetoothSupportInfo(locale) {
    const isVi = locale === "vi";
    if (typeof window === "undefined") {
        return {
            available: false,
            label: isVi ? "Khong ho tro" : "Unsupported",
            detail: isVi
                ? "Moi truong nay khong co trinh duyet de mo Bluetooth."
                : "This environment does not expose a browser Bluetooth API.",
        };
    }
    ...
    if (!isSecure) {
        return {
            ...
            label: "Can HTTPS",
            detail: isVi
                ? "Web Bluetooth chi chay tren HTTPS hoac localhost. Hay mo webtool bang HTTPS."
                : "Web Bluetooth only works on HTTPS or localhost. Open this webtool from a secure URL.",
        };
    }
    if (typeof nav.bluetooth !== "undefined") {
        return {
            available: true,
            label: "Web Bluetooth",
            detail: isVi
                ? "Trinh duyet san sang ket noi Bluetooth LE."
                : "This browser is ready to connect to Bluetooth LE devices.",
        };
    }
    ...
    if (isIos) {
        return {
            ...
            label: "iOS WebKit",
            detail: isVi
                ? "Safari/Chrome tren iPhone va iPad khong co Web Bluetooth. Hay dung Chrome hoac Edge tren Windows/macOS/Android, hoac trinh duyet WebBLE rieng tren iOS."
                : "Safari and Chrome on iPhone/iPad do not expose Web Bluetooth. Use Chrome or Edge on Windows/macOS/Android, or a dedicated WebBLE browser on iOS.",
        };
    }
    if (isSafari) {
        return {
            ...
            label: "Safari",
            detail: isVi
                ? "Safari hien khong ho tro Web Bluetooth, nen webtool khong the mo hop chon BLE trong Safari. Hay dung Chrome hoac Edge."
                : "Safari does not currently support Web Bluetooth, so this webtool cannot open the BLE chooser in Safari. Use Chrome or Edge.",
        };
    }
    return {
        available: false,
        label: isVi ? "Khong ho tro" : "Unsupported",
        detail: isVi
            ? "Trinh duyet nay khong co Web Bluetooth. Hay dung Chrome hoac Edge ban moi."
            : "This browser does not expose Web Bluetooth. Use a recent Chrome or Edge browser.",
    };
}
function describeBluetoothError(error, locale) {
    const isVi = locale === "vi";
    if (error instanceof DOMException) {
        if (error.name === "NotFoundError") {
            return isVi
                ? "Ban da huy chon thiet bi hoac khong co thiet bi phu hop."
                : "Device selection was canceled or no matching device was found.";
        }
        if (error.name === "SecurityError" || error.name === "NotAllowedError") {
            return isVi
                ? "Quyen Bluetooth bi chan. Kiem tra HTTPS, quyen trinh duyet va thao tac bam nut ket noi."
                : "Bluetooth permission was blocked. Check HTTPS, browser permissions, and start from the connect button.";
        }
        if (error.name === "NetworkError") {
            return isVi
                ? "Khong ket noi duoc GATT. Hay bat lai Bluetooth/thiet bi roi thu lai."
                : "Could not connect to GATT. Restart Bluetooth/the device and try again.";
        }
    }
...
function App() {
    ...
    const bluetoothSupport = useMemo(() => getBluetoothSupportInfo(locale), [locale]);
    const bluetoothAvailable = bluetoothSupport.available;
    ...
    const handleConnect = async () => {
        ...
        if (!bluetoothAvailable) {
            appendStatus(bluetoothSupport.detail);
            return;
        }
        setConnecting(true);
        try {
            ...
            if (!bluetooth) {
                throw new Error(bluetoothSupport.detail);
            }
            appendStatus("Dang mo hop chon thiet bi Bluetooth...");
            ...
            for (let retries = 3; retries > 0; retries -= 1) {
                try {
                    ...
                    if (!device.gatt) {
                        throw new Error("Thiet bi khong ho tro GATT.");
                    }
            ...
            appendStatus(`Kết nối thất bại: ${describeBluetoothError(error, locale)}`);
            resetConnection();
        }
        finally {
            setConnecting(false);
            ...
            React.createElement("div", { className: "ble-console" },
                React.createElement("div", { className: "ble-console-top" },
                    ...
                    React.createElement("div", { className: "ble-console-copy" },
                        ...
                        !props.bluetoothAvailable && (React.createElement("p", { className: "ble-support-note" }, props.bluetoothSupport.detail)))),
                React.createElement("div", { className: "ble-console-stats" },
                    ...
                    React.createElement("div", { className: "ble-stat" },
                        ...
                        React.createElement("strong", null, props.bluetoothSupport.label))),
                React.createElement("div", { className: "ble-console-actions" },
                    React.createElement("button", { className: "primary-button", disabled: props.connected || props.connecting || !props.bluetoothAvailable, onClick: props.onConnect }, props.connected
                        ? isVi
                            ? "Đã kết nối"
                            : "Connected"
                        : props.connecting
